...
- (void)configSubViews
{
    ...
    [_addressLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(_addressIcon.right).offset(kDefaultPadding);
        make.centerY.mas_equalTo(self);
    }];
...
{
    if (!_addressIcon) {
        _addressIcon = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"zhq_shop_place.png"]];
    }
    return _addressIcon;
}
- (UIButton *)phoneButton
{
    if (!_phoneButton) {
        ...
        [_phoneButton setImage:[UIImage imageNamed:@"zhq_shop_phone.png"] forState:UIControlStateNormal];
