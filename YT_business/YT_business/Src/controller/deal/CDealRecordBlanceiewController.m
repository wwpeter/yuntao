...
#import "CMobileRechargeViewController.h"
...
@interface CDealRecordBlanceiewController ()
...
@implementation CDealRecordBlanceiewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.requestURL = queryAccountURL;
}
                     ...
                     error:(NSError*)requestErr
{
    ...
    if (parserObject.success) {
        if ([operation.urlTag isEqualToString:queryAccountURL]) {
        ...
        else if ([operation.urlTag isEqualToString:steupPayPasswdURL]) {
...
{
    CDealRecordBlanceiewDetailViewController* dealRecordBlanceiewDetailVC = [[CDealRecordBlanceiewDetailViewController alloc] init];
...
- (void)rechargeButtonClicked:(id)sender
{
    if ([NSStrUtil isEmptyOrNull:[YTUsr usr].payPwd]) {
        ...
        CMobileRechargeViewController* mobileRechargeVC = [[CMobileRechargeViewController alloc] init];
        [self.navigationController pushViewController:mobileRechargeVC animated:YES];
    }
}
- (void)extractButtonClicked:(id)sender
{
    //    CWithdrawViewController* withdrawVC = [[CWithdrawViewController alloc] init];
    //    [self.navigationController pushViewController:withdrawVC animated:YES];
    if ([NSStrUtil isEmptyOrNull:[YTUsr usr].payPwd]) {
        [self setupUserPayPassword];
    }
    else {
        CBalancesWithdrawViewController* balancesWithdrawVC = [[CBalancesWithdrawViewController alloc] initWithRemain:self.accountModel.account.remain];
