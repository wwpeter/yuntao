...
@implementation CDealRecordTableCell
...
- (void)configDealTableCellWithIntroModel:(YTTrade *)trade {
    ...
    self.payLabel.text = [YTTaskHandler outPayTyoeStrWithType:trade.payType];
    self.orderLabel.text = [NSString stringWithFormat:@"订单号:%@",trade.toOuterId];
    self.priceLabel.text = [NSString stringWithFormat:@"￥%.2f",trade.originPrice/100.];
    self.timeLabel.text = [YTTaskHandler outDateStrWithTimeStamp:trade.createdAt withStyle:@"yyyy-MM-dd"];
}
- (void)configDealTableCellWithAccountDetailModel:(YTAccountDetail *)accountDetail
{
    ...
    self.timeLabel.text = [YTTaskHandler outDateStrWithTimeStamp:accountDetail.createdAt withStyle:@"yyyy-MM-dd"];
}
