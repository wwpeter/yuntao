...
@interface CMobileRechargeViewController ()<UITextFieldDelegate,ABPeoplePickerNavigationControllerDelegate>
...
@implementation CMobileRechargeViewController
                     ...
                     error:(NSError*)requestErr
{
    if (parserObject.success) {
        if ([operation.urlTag isEqualToString:mobileChargeURL]) {
            HbPaySuccessViewController* successVC = [StoryBoardUtilities viewControllerForMainStoryboard:[HbPaySuccessViewController class]];
            successVC.hideButton = YES;
            successVC.paySuccessTitle = @"充值成功";
            successVC.paySuccessDescribe = [NSString stringWithFormat:@"%@,充值金额%@", self.textField.text, self.amount];
            [self.navigationController pushViewController:successVC animated:YES];
        }
    }
    else {
        if ([operation.urlTag isEqualToString:mobileChargeURL]) {
            if ([parserObject.message isEqualToString:@"支付密码不正确"]) {
        ...
        tradeView.finish = ^(NSString *passWord){
           __strong __typeof(wSelf)strongSelf = wSelf;
            strongSelf.requestParas = @{@"mobile" : strongSelf.textField.text,
                                        ...
                                        loadingKey : @(YES)};
            strongSelf.requestURL = mobileChargeURL;
        };
        [tradeView show];
    }
}
