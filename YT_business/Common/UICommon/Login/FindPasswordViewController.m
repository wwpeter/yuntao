...
- (void)registerButtonClicked:(UIButton*)sender
{
    ...
    if ([self checkTextDidInEffect]) {
        ...
        if (_loginType == LoginViewTypeAsstanter) {
            self.requestURL = saleUpdatePwdForForgetURL;
        }
        else {
            self.requestURL = updatePwdForForgetURL;
        }
...
- (void)actionFetchRequest:(AFHTTPRequestOperation *)operation result:(YTBaseModel *)parserObject
    ...
    if (parserObject.success) {
        if ([operation.urlTag isEqualToString:updatePwdForForgetURL] ||
            [operation.urlTag isEqualToString:saleUpdatePwdForForgetURL]) {
...
{
    if (_phoneField.text.length > 10) {
        self.requestParas = @{@"mobile" : _phoneField.text};
        if (_loginType == LoginViewTypeAsstanter) {
            self.requestURL = saleSendSmsCodeForForgetPwdURL;
        }
        else {
            self.requestURL = sendSmsCodeForForgetPwdURL;
        }
    }
    else {
