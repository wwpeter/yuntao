...
#pragma mark - 修改尺寸
- (UIImage*)imageScaledWithMaxWidth:(CGFloat)width
{
    ...
    CGFloat multiple = width / imageWidth;
    imageHeight = imageHeight * multiple;
    imageWidth = width;
    return [self imageWithImage:self scaledToSize:CGSizeMake(imageWidth, imageHeight)];
}
...
+(UIImage *)addWatermarkImage:(UIImage *)watermarkImage backgroundImage:(UIImage *)backgroundImage 
{
    UIGraphicsBeginImageContext(backgroundImage.size);
    [backgroundImage drawInRect:CGRectMake(0, 0, backgroundImage.size.width, backgroundImage.size.height)];
//    [watermarkImage drawInRect:CGRectMake(backgroundImage.size.width - watermarkImage.size.width, backgroundImage.size.height - watermarkImage.size.height, watermarkImage.size.width, watermarkImage.size.height)];
    CGFloat bw = backgroundImage.size.width;
    CGFloat bh = backgroundImage.size.height;
    CGFloat ww = watermarkImage.size.width;
    CGFloat wh = watermarkImage.size.height;
    
    [watermarkImage drawInRect:(CGRect){{bw/2-ww/4, bh/2-wh/4}, {ww/2, wh/2}}];
