...
#pragma mark 周转数字
+ (NSString*)makeOpendaysNumberWithDays:(NSString*)days
{
    NSArray* dayArray = [days componentsSeparatedByString:@","];
    NSMutableArray* valueArr = [NSMutableArray arrayWithCapacity:dayArray.count];
    [valueArr sortUsingSelector:@selector(compare:)];
    for (NSString* dateStr in dayArray) {
        if ([dateStr isEqualToString:@"周一"]) {
            [valueArr addObject:@"1"];
        }
        else if ([dateStr isEqualToString:@"周二"]) {
            [valueArr addObject:@"2"];
        }
        else if ([dateStr isEqualToString:@"周三"]) {
            [valueArr addObject:@"3"];
        }
        else if ([dateStr isEqualToString:@"周四"]) {
            [valueArr addObject:@"4"];
        }
        else if ([dateStr isEqualToString:@"周五"]) {
            [valueArr addObject:@"5"];
        }
        else if ([dateStr isEqualToString:@"周六"]) {
            [valueArr addObject:@"6"];
        }
        else if ([dateStr isEqualToString:@"周日"]) {
            [valueArr addObject:@"7"];
        }
    }
    if (valueArr.count == 1) {
        return [valueArr lastObject];
    }
...
+ (NSString*)makeOpendaysWeekWithDays:(NSString*)days
{
    NSArray* dayArray = [days componentsSeparatedByString:@","];
    NSArray*sortedArray = [dayArray sortedArrayUsingSelector:@selector(compare:)];
    NSMutableArray* valueArr = [NSMutableArray arrayWithCapacity:sortedArray.count];
    [valueArr sortUsingSelector:@selector(compare:)];
    for (NSString* dateStr in dayArray) {
        ...
        else if ([dateStr isEqualToString:@"4"]) {
            [valueArr addObject:@"周四"];
        }
        else if ([dateStr isEqualToString:@"5"]) {
            [valueArr addObject:@"周五"];
        }
        else if ([dateStr isEqualToString:@"6"]) {
