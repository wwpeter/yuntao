...
//    [bezierPath stroke];
    ...
    bezierPath = [UIBezierPath bezierPath];
    [bezierPath moveToPoint:CGPointMake(CGRectGetWidth(rect), 5.0)];
    [bezierPath addLineToPoint:CGPointMake(CGRectGetWidth(rect), CGRectGetHeight(rect)-5.0)];
    [[UIColor colorWithWhite:197.0/255.0 alpha:0.75] setStroke];
    [bezierPath setLineWidth:1.0];
    [bezierPath stroke];
    ...
    if (self.selected) {
        ...
        // change title color
        self.titleLabel.textColor = [UIColor colorWithRed:255.0/255.0 green:89.0/255.0 blue:96.0/255.0 alpha:1];
    }
}
@end
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[self.tabs objectAtIndex:index] isEqual:[NSNull null]]) {
         ...
         NSString *title = [self.dataSource viewPager:self titleForTabAtIndex:index];
