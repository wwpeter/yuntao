...
- (void)pullToRefreshForTableView
{
    self.pullToRefreshView.state = SVPullToRefreshStateTriggered;
    [self.pullToRefreshView startAnimating];
}
- (void)triggerPullToRefresh {
    [self performSelector:@selector(pullToRefreshForTableView) withObject:nil afterDelay:0.5f];
}
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(@"下拉刷新...",@"Pull to refresh..."),
                             NSLocalizedString(@"放开立即刷新...",@"Release to refresh..."),
                             NSLocalizedString(@"请稍后...",@"Loading..."),
                                nil];
...
- (UIImageView *)arrow {
    if(!_arrow) {
        //        CGFloat arrowX = (self.bounds.size.width / 2) - (totalMaxWidth / 2) + (leftViewWidth - self.arrow.bounds.size.width) / 2;
        _arrow = [[UIImageView alloc]initWithFrame:CGRectMake((self.bounds.size.width-18)/2, (self.bounds.size.height-25)/2, 16, 27)];
...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        ...
        _titleLabel.text = NSLocalizedString(@"下拉刷新...",@"Pull to refresh...");
...
{
    self.arrow.image = newArrowImage; // pass through
...
- (void)setLastUpdatedDate:(NSDate *)newLastUpdatedDate {
    ...
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"上次刷新: %@",@"Last Updated: %@"), newLastUpdatedDate?[self.dateFormatter stringFromDate:newLastUpdatedDate]:NSLocalizedString(@"Never",)];
}

- (void)setDateFormatter:(NSDateFormatter *)newDateFormatter {
	dateFormatter = newDateFormatter;
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"上次刷新: %@",@"Last Updated: %@"), self.lastUpdatedDate?[newDateFormatter stringFromDate:self.lastUpdatedDate]:NSLocalizedString(@"Never",)];
}
