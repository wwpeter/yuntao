...
- (UILabel *)endLabel {
    if(!_endLabel) {
        _endLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.frame), 30)];
        _endLabel.font = [UIFont systemFontOfSize:14];
        _endLabel.textAlignment = NSTextAlignmentCenter;
        _endLabel.text = @"已经到底部了>_<";
...
- (void)endScrollAnimating {
    [UIView animateWithDuration:0.3
                     ...
                     completion:^(BOOL finished) {
                         ...
                         [self performSelector:@selector(hideEndLabel) withObject:nil afterDelay:2.5];
                     }];
}
