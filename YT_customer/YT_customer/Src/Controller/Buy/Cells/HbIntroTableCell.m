...
@implementation HbIntroTableCell
...
{
    if (introModel.hbStatus == HbIntroModelStatusExpired || introModel.hbStatus == HbIntroModelStatusClosed) {
    ...
    self.timeLabel.text = introModel.userUseEndDateStr;
    self.hbStatusImageView.image = [UIImage imageNamed:introModel.hbStatusImageName];
    [self.hbImageView sd_setImageWithURL:[introModel.imageUrl imageUrlWithWidth:200]placeholderImage:YTNormalPlaceImage];
    self.nameLabel.text = [NSString stringWithFormat:@"%@",introModel.hbName];
    self.describeLabel.text = introModel.shopName;
    self.priceLabel.text = [NSString stringWithFormat:@"￥%@",introModel.cost];
    if(introModel.isUserHB)
    {
        self.hbStatusImageView.image = [UIImage imageNamed:@""];
        self.timeLabel.text = @"未使用";
    }
}
