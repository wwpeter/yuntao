...
@interface CSearchStoreViewController () <UISearchBarDelegate, UITableViewDataSource, UITableViewDelegate>
@property (strong, nonatomic) UISearchBar* searchBar;
@property (strong, nonatomic) UITableView* tableView;
@property (strong, nonatomic) NSMutableArray* dataArray;
@end

@implementation CSearchStoreViewController
...
- (void)searchBarSearchButtonClicked:(UISearchBar*)searchBar
{
    ...
    if([self.dataArray indexOfObject:searchBar.text] == NSNotFound) {
        ...
        [CacheDataUtil saveData:self.dataArray withFileName:iYCUserSearchStoreData];
    }
...
- (void)cleanFooterViewTap:(UITapGestureRecognizer*)tap
{
    ...
    [alert setCompletionBlock:^(UIAlertView* alertView, NSInteger buttonIndex) {
        ...
        if (buttonIndex == 1) {
            ...
            [CacheDataUtil cleanCacheWithFileName:iYCUserSearchStoreData];
        }
    }];
    [alert show];
}
...
{
    CSearchStoreResultViewController* searchResultVC = [[CSearchStoreResultViewController alloc] initWithStyle:UITableViewStylePlain];
    searchResultVC.keyword = keyword;
    searchResultVC.searchResultModule = self.searchResultModule;
...
- (void)initializePageSubviews
{
    ...
    if (self.searchResultModule == SearchResultModuleStore) {
        _searchBar.placeholder = @"搜索商户";
    }else{
        _searchBar.placeholder = @"搜索红包";
    }
    NSArray* cacheData = [CacheDataUtil loadCacheObjectWithFileName:iYCUserSearchStoreData];
    if (cacheData) {
