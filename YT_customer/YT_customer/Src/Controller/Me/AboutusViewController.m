...
static NSString *const YT_Service_Strategy = @"http://user.strategy.yuntaohongbao.com";
...
{
    if (!_iconImageView) {
         _iconImageView  = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"yt_customer_logo_02.png"]];
    }
...
{
    if (!_agreementView) {
        _agreementView = [[RGActionView alloc] initWithTitle:@"云淘红包用户协议" frame:CGRectMake(0, _actionViewHeight, kDeviceWidth, 50)];
...
{
    if (!_strategyView) {
        _strategyView = [[RGActionView alloc] initWithTitle:@"云淘红包用户攻略" frame:CGRectMake(0, _actionViewHeight+50, kDeviceWidth, 50)];
