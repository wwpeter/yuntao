...
- (id)initWithTitle:(NSString*)title image:(UIImage*)image
{
    ...
    if (self) {
        ...
        [self setTitleColor:BaseMenuTextColor forState:UIControlStateNormal];
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.backgroundColor = BaseMenuBackgroundColor;
        ...
        _titleLabel.textColor = BaseMenuTextColor;
        ...
        [_cancelButton setTitleColor:BaseMenuTextColor forState:UIControlStateNormal];
...
- (void)layoutSubviews
{
    ...
    CGPoint titlePoint = CGPointMake(0, 10);
    self.titleLabel.frame = (CGRect){ titlePoint, CGSizeMake(self.bounds.size.width, 30) };
    ...
    self.cancelButton.frame = CGRectMake(0, self.titleLabel.bounds.size.height + self.contentScrollView.bounds.size.height, self.bounds.size.width, 50);
...
{
    UIEdgeInsets margin = UIEdgeInsetsMake(0, 10, 20, 10);
    CGSize itemSize = CGSizeMake((self.bounds.size.width - margin.left - margin.right) / 3, 116);
