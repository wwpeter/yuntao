...
- (void)setupInputView
{
    ...
    inputView.inputTitle = _inputTitle;
...
- (void)showKeyboard
{
    ...
    [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        self.keyboard.transform = CGAffineTransformMakeTranslation(0, -self.keyboard.zc_height);
        self.inputView.transform = CGAffineTransformMakeTranslation(0, marginTop - self.inputView.zc_y);
...
- (void)cancle
{
    [self hidenKeyboard:^(BOOL finished) {
        ...
        [UIAlertView showWithTitle:@"是否取消?" message:nil delegate:self cancelButtonTitle:@"否" otherButtonTitles:@"是", nil];
    }];
}
...
- (void)showInView:(UIView *)view
{
    ...
    /** 键盘起始frame */
    self.keyboard.zc_x = 0;
    self.keyboard.zc_y = ZCScreenHeight;
    self.keyboard.zc_width = ZCScreenWidth;
    self.keyboard.zc_height = ZCScreenWidth * 0.65;
    
    /** 输入框起始frame */
    self.inputView.zc_height = ZCScreenWidth * 0.5625;
    self.inputView.zc_y = (self.zc_height - self.inputView.zc_height) * 0.5;
    self.inputView.zc_width = ZCScreenWidth * 0.94375;
    self.inputView.zc_x = (ZCScreenWidth - self.inputView.zc_width) * 0.5;
