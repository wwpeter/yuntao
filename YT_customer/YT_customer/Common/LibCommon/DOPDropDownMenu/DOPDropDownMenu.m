...
#define kTextSelectColor [UIColor colorWithRed:253/255.0 green:92/255.0 blue:99/255.0 alpha:1]
...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        //indicator
        CAShapeLayer *indicator = [self createIndicatorWithColor:self.indicatorColor andPosition:CGPointMake((i + 1)*separatorLineInterval - 18, self.frame.size.height / 2)];
...
//        if ([cell.textLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]])
        ...
        if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:indexPath.row column:_currentSelectedMenudIndex]> 0){
            ...
            cell.accessoryView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"yt_store_cleanAccess"] highlightedImage:[UIImage imageNamed:@"yt_store_redAccess"]];
        }
        ...
        if ([cell.textLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
            ...
            cell.accessoryView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"yt_store_redAccess"]];
        } else {
